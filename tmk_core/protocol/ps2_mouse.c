...
void ps2_mouse_task(uint8_t middle)
{
    ...
    if (rcv == PS2_ACK) {
		...
		// ISS: send middle button with mouse data (from key matrix)
		if (middle > 0) {
			if (debug_mouse) print("ISS ps2_mouse hack: add pressed middle button\n");
			mouse_report.buttons |= (1<<PS2_MOUSE_BTN_MIDDLE);
		}
#endif
    ...
    if (mouse_report.x || mouse_report.y ||
		...
		// ISS: Removed scroll button handling to get "normal middle button"
